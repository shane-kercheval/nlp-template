...
class CorpusDataLoader(DataPersistence):
    ...
    async def _save_async(self, data):
        tasks = []
        for index, doc_dict in enumerate(data.to_doc_dicts()):
            file_name = self._get_file_name(index=index)
    ...
    def _save(self, data: Corpus):
        ...
        self.loop.run_until_complete(self._save_async(data))
    ...
    def _load(self) -> list[dict]:
        logging.info(f"Load Corpus object from `{self.sub_directory}`")
