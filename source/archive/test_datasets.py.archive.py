...
def save_json(dict_val: dict, file_name: str):
...
class DictList:
    ...
    def save(self, file_prefix):
        self.loop.run_until_complete(self._save_async(file_prefix))

    async def _save_async(self, file_prefix):
        tasks = []
        for i, dictionary in enumerate(self.dicts):
            filename = f"{file_prefix}_{i}.json"
            task = self.loop.run_in_executor(None, save_json, dictionary, filename)
            tasks.append(task)
        await asyncio.gather(*tasks)

    def load(self, file_prefix):
        results = self.loop.run_until_complete(self._load_async(file_prefix))
        self.dicts = results

    async def _load_async(self, file_prefix):
        tasks = []
        for i in range(3):
            filename = f"{file_prefix}_{i}.json"
            task = self.loop.run_in_executor(None, load_json, filename)
            tasks.append(task)
        return await asyncio.gather(*tasks)


# def test_asdfasdf():
#     # Create a DictList object
#     dict_list = DictList()

#     # Add some dictionaries to the list
#     dict_list.dicts = [
#         {"name": "Alice", "age": 30},
#         {"name": "Bob", "age": 40},
#         {"name": "Charlie", "age": 50}
...
#     # Save the list to JSON files
#     # dict_list.save("data")

#     # Clear the list
#     dict_list.dicts = []
#     # Load the list from JSON files
#     dict_list.load("data")
#     dict_list.dicts

#     # Print the list of dictionaries
#     print(dict_list.dicts)
